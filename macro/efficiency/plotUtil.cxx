...
TString datalbl = "Data, 4.9 fb^{-1}", reflbl = "Simulation";
...
void doRatio(TGraphAsymmErrors *hfit, TGraphAsymmErrors *href, TString alias, const char *xtitle, TPaveText *comment) {
  gStyle->SetOptStat(0);
    ...
    TH1F *frame = new TH1F("frameratio","",1, ratio.GetX()[0]-ratio.GetErrorXlow(0), TMath::Min(ratio.GetX()[ratio.GetN()-1]+ratio.GetErrorXhigh(ratio.GetN()-1),80.0));
    frame->Draw("");
    ...
    TLine line(ratio.GetX()[0]-ratio.GetErrorXlow(0), 1, 80., 1);
    line.SetLineWidth(2);
...
void refstack(TDirectory *fit, TDirectory *refd, TString alias, TString fitname) {
    ...
    if (doFillMC) {
        href->SetLineColor(62);
        href->SetFillColor(65);
        href->SetLineStyle(0);
        href->SetMarkerColor(62);
    ...
    if (doRatioPlot) doRatio(hfit,href,alias,getXtitle(pfit),0); 
