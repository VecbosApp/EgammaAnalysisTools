...
int main(int argc, char* argv[]) {
  ...
  for(int iecal=0; iecal<2; iecal++) {
    ...
    for(int iclass=0; iclass<2; iclass++) {
      ...
      dPhiVtxClassEle1[iecal][iptbin][iclass]     = (TH1F*)gDirectory->Get(histo);
      ...
      sigmaEtaEtaClassEle1[iecal][iptbin][iclass] = (TH1F*)gDirectory->Get(histo); 
  ...
  signalFile2->cd("pdfsProducer");

  TH1F *dPhiVtxClassEle2[2][2][2];
  TH1F *dEtaClassEle2[2][2][2];
  TH1F *EoPoutClassEle2[2][2][2];
  TH1F *HoEClassEle2[2][2][2];
  TH1F *sigmaEtaEtaClassEle2[2][2][2];
  ...
  for(int iecal=0; iecal<2; iecal++) {
    ...
    for(int iclass=0; iclass<2; iclass++) {
      ...
      sprintf(histo,"dPhiVtxClass_electrons_%d_%d_%d",iecal,iptbin,iclass);
      dPhiVtxClassEle2[iecal][iptbin][iclass]     = (TH1F*)gDirectory->Get(histo);
      sprintf(histo,"dEtaClass_electrons_%d_%d_%d",iecal,iptbin,iclass);
      ...
      sprintf(histo,"sigmaEtaEtaClass_electrons_%d_%d_%d",iecal,iptbin,iclass);
      sigmaEtaEtaClassEle2[iecal][iptbin][iclass] = (TH1F*)gDirectory->Get(histo); 
      sprintf(histo,"s9s25Class_electrons_%d_%d_%d",iecal,iptbin,iclass);
  ...
  makePlots(dPhiVtxClassEle1,dPhiVtxClassEle2,"dPhiIn","#Delta #phi_{in} (rad)");
  ...
  makePlots(sigmaEtaEtaClassEle1,sigmaEtaEtaClassEle2,"sigmaEtaEta","#sigma_{#eta#eta} (rad^{2})");
...
//     backgroundHistos[iecal][iptbin]->SetMaximum(max+sqrt(max));
    ...
    signalHistos2[iecal][iptbin][0]->Draw("same hist");
    signalHistos2[iecal][iptbin][1]->Draw("same hist");

    TLegend* leg = new TLegend(0.15,0.75,0.40,0.90);
    leg->SetFillStyle(0); leg->SetBorderSize(0); leg->SetTextSize(0.03);
    leg->SetFillColor(0);
    leg->AddEntry(signalHistos1[iecal][iptbin][0],"Summer08 non-showering");
    leg->AddEntry(signalHistos1[iecal][iptbin][1],"Summer08 showering");
    leg->AddEntry(signalHistos2[iecal][iptbin][0],"CSA07 non-showering");
    leg->AddEntry(signalHistos2[iecal][iptbin][1],"CSA07 showering");
    leg->Draw();
