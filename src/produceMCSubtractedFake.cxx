...
int main(int argc, char* argv[]) {
  ...
  // input files
  TFile *filePT_EB[3];
  filePT_EB[0] = new TFile("results_data/EleMisidPtBarrel.root");
  // filePT_EB[0] = new TFile("macro/fakeTagAndProbe/weightedHistos_1p_mergedSel_orHLT_ET30_newDenomAfterEPS/mergedResultPtBarrelQCD.root");
  filePT_EB[1] = new TFile("macro/fakeTagAndProbe/weightedHistos_1p_mergedSel_orHLT_ET35_newDenomAfterEPS/mergedResultPtBarrelWjets.root");
  filePT_EB[2] = new TFile("macro/fakeTagAndProbe/weightedHistos_1p_mergedSel_orHLT_ET35_newDenomAfterEPS/mergedResultPtBarrelDY.root");

  TFile *filePT_EE[3];
  filePT_EE[0] = new TFile("results_data/EleMisidPtEndcap.root");
  // filePT_EE[0] = new TFile("macro/fakeTagAndProbe/weightedHistos_1p_mergedSel_orHLT_ET30_newDenomAfterEPS/mergedResultPtEndcapQCD.root");
  filePT_EE[1] = new TFile("macro/fakeTagAndProbe/weightedHistos_1p_mergedSel_orHLT_ET35_newDenomAfterEPS/mergedResultPtEndcapWjets.root");
  filePT_EE[2] = new TFile("macro/fakeTagAndProbe/weightedHistos_1p_mergedSel_orHLT_ET35_newDenomAfterEPS/mergedResultPtEndcapDY.root");
  ...
  for (int ii=0; ii<3; ii++) { 
    ...
    // LHIdLoosePt_PRE_EB[ii]    = (TH1F*)filePT_EB[ii]->Get("LHIdLHLoosePtBarrel");
    // LHIdLoosePt_PRE_EB[ii]    = (TH1F*)filePT_EB[ii]->Get("LHIdLHLoosePFIsoPtBarrel");
    LHIdLoosePt_PRE_EB[ii]    = (TH1F*)filePT_EB[ii]->Get("CutIdWP80SmurfPtBarrel");
    FakeableJetsPt_PRE_EB[ii] = (TH1F*)filePT_EB[ii]->Get("RecoPtBarrel");

    // LHIdLoosePt_PRE_EE[ii]    = (TH1F*)filePT_EE[ii]->Get("LHIdLHLoosePtEndcap");
    // LHIdLoosePt_PRE_EE[ii]    = (TH1F*)filePT_EE[ii]->Get("LHIdLHLoosePFIsoPtEndcap");
    LHIdLoosePt_PRE_EE[ii]    = (TH1F*)filePT_EE[ii]->Get("CutIdWP80SmurfPtEndcap");
    FakeableJetsPt_PRE_EE[ii] = (TH1F*)filePT_EE[ii]->Get("RecoPtEndcap");
  }
  ...
  // TH1F *LHIdLoosePt_EB = (TH1F*)LHIdLoosePt_PRE_EB[0] -> Clone("LHIdLoosePt_EB");
  // TH1F *LHIdLoosePt_EB = (TH1F*)LHIdLoosePt_PRE_EB[0] -> Clone("LHIdLoosePFIsoPt_EB");
  TH1F *LHIdLoosePt_EB = (TH1F*)LHIdLoosePt_PRE_EB[0] -> Clone("CutIdWP80SmurfPt_EB");
  for (int ibin=0; ibin<7; ibin++) {
  ...
  // TH1F *LHIdLoosePt_EE = (TH1F*)LHIdLoosePt_PRE_EE[0] -> Clone("LHIdLoosePt_EE");
  // TH1F *LHIdLoosePt_EE = (TH1F*)LHIdLoosePt_PRE_EE[0] -> Clone("LHIdLoosePFIsoPt_EE");
  TH1F *LHIdLoosePt_EE = (TH1F*)LHIdLoosePt_PRE_EE[0] -> Clone("CutIdWP80SmurfPt_EE");
  for (int ibin=0; ibin<7; ibin++) {
  ...
  for (int ibin=0; ibin<7; ibin++) cout << "data: bin " << ibin << ", content = " << LHIdLoosePt_PRE_EB[0]->GetBinContent(ibin) << endl;
  cout << endl;
  for (int ibin=0; ibin<7; ibin++) cout << "Wj:   bin " << ibin << ", content = " << LHIdLoosePt_PRE_EB[1]->GetBinContent(ibin) << endl;
  cout << endl;
  for (int ibin=0; ibin<7; ibin++) cout << "DY:   bin " << ibin << ", content = " << LHIdLoosePt_PRE_EB[2]->GetBinContent(ibin) << endl;
  cout << endl;
  cout << "after the subtraction" << endl;
  for (int ibin=0; ibin<7; ibin++) cout << "bin " << ibin << ", content = " << LHIdLoosePt_EB->GetBinContent(ibin) << endl;
  cout << endl;
  */
  
  // now computing the fake rate - before and after subtraction
  EfficiencyEvaluator ElectronEfficiencyPtLH_EB("elefake-pt-barrel.root");
  ...
  EfficiencyEvaluator ElectronEfficiencyPtLH_EE("elefake-pt-endcap.root");
