...
else{$workdir="/cmshome/dimarcoe/releases/LIKELIHOOD_RELEASES/OfflineAnalysis/EgammaAnalysisTools";}
if($opt_C){$CMSSWdir=$opt_C;}
else {$CMSSWdir="/cmshome/dimarcoe/releases/LIKELIHOOD_RELEASES/CMSSW_2_1_12";}
if($opt_f){$file = $opt_f;}
else{$file="/cmshome/dimarcoe/releases/LIKELIHOOD_RELEASES/OfflineAnalysis/datasets/QCD/QCD_EMenriched_Pt20to30_datasets.txt";}
...
# create the scripts to submit
    ...
    $outputfile = "$rootdir/job-$j-pdfs.root";
    open(SCRIPTFILE,">$scriptfile");
    print SCRIPTFILE "\#\!/bin/tcsh\n\n";
    print SCRIPTFILE "cd $CMSSWdir\n";
    print SCRIPTFILE "eval `scramv1 runtime -csh`\n";
    print SCRIPTFILE "cd -\n";
    print SCRIPTFILE "setenv X509_USER_PROXY /cmshome/dimarcoe/certtemp/gpi.out\n";
    print SCRIPTFILE "$workdir/EgammaAnalysis $jobfile $outputfile\n";
